...
import { Wand2, LayoutPanelLeft, Clock, History, Save, Copy, Check, ArrowRight, Sparkles, MessageSquare, Plus, Trash2, ChevronUp, ChevronDown, Lock, Unlock, Play, RefreshCw } from 'lucide-react';

const mockScenes = [
  { id: 1, time: '0:00 - 0:03', visual: 'Quay cận mặt, biểu cảm giật mình hoảng hốt.', dialogue: 'Bạn đã bao giờ làm crash trình duyệt chỉ vì một cái useEffect viết sai chưa?', text: 'Lỗi useEffect ám ảnh', effect: 'Zoom in nhanh, rung lắc nhẹ', sound: 'Hiệu ứng "Whoosh", tiếng kính vỡ nhẹ' },
  { id: 2, time: '0:03 - 0:10', visual: 'Chuyển cảnh nhanh sang màn hình code đầy lỗi đỏ.', dialogue: 'Đừng lo, 90% anh em dev React đều từng dính chưởng này khi mới học Hooks.', text: '90% Dev đều mắc phải', effect: 'Hiệu ứng Glitch chuyển cảnh', sound: 'Nhạc nền lofi beat bắt đầu' },
  { id: 3, time: '0:10 - 0:25', visual: 'Vẽ sơ đồ đơn giản lên màn hình, chỉ tay giải thích.', dialogue: 'Bí kíp sống còn số 1: Luôn luôn kiểm soát dependency array. Nếu bạn để trống, nó chạy 1 lần. Nếu bỏ ngoặc vuông, nó chạy infinite loop!', text: '[ ] = 1 Lần\nNo [ ] = Infinity', effect: 'Text pop up theo nhịp nói', sound: 'Tiếng pop pop khi chữ hiện' },
  { id: 4, time: '0:25 - 0:35', visual: 'So sánh 2 đoạn code bên trái và phải màn hình.', dialogue: 'Hãy xem cách refactor đoạn code lộn xộn này thành một Custom Hook siêu sạch sẽ.', text: 'Before & After', effect: 'Chia đôi màn hình (Split screen)', sound: 'Tiếng ting (thành công)' },
  ...
  { id: 6, time: '0:45 - 0:50', visual: 'Chỉ tay xuống dưới màn hình.', dialogue: 'Lưu ngay video này lại để xem khi debug nhé! Và follow mình để học thêm về React.', text: 'Lưu & Follow!', effect: 'Hiệu ứng mũi tên chỉ xuống, icon tym bay', sound: 'Tiếng chuông notification' },
];
    ...
    <div className="h-[calc(100vh-8rem)] flex flex-col md:flex-row gap-6 overflow-hidden">
      ...
      <div className={`flex-shrink-0 flex flex-col bg-[#1A1533]/50 border border-white/5 rounded-xl overflow-hidden transition-all duration-500 ease-in-out ${isGenerated ? 'w-full md:w-80' : 'w-full max-w-2xl mx-auto'}`}>
        <div className="p-4 border-b border-white/5 bg-[#0F0B1A]/50 flex items-center justify-between">
          ...
          {isGenerated && <Button variant="ghost" size="sm" className="h-8 text-xs">Thu gọn</Button>}
        </div>
        ...
        <div className="p-4 overflow-y-auto flex-1 custom-scrollbar space-y-5">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button variant="outline" className="bg-[#0F0B1A] border-white/10 text-gray-400">YouTube Dài</Button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-gray-400">Giọng điệu (Tone & Voice)</label>
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-gray-400">Góc nhìn / Ý tưởng mới (Optional)</label>
            <Textarea 
              placeholder="Ví dụ: Thêm một ví dụ thực tế về dự án e-commerce bị lỗi hook..."
          ...
          <div className="pt-2">
            ...
            >
              {isGenerating ? (
                ...
                <span className="flex items-center"><Sparkles className="w-4 h-4 mr-2" /> Tạo kịch bản AI</span>
              )}
            </Button>
          </div>
        </div>
          ...
          <div className="p-4 border-b border-white/10 flex justify-between items-center bg-[#1A1533]/80">
            <div>
              <h2 className="text-lg font-bold text-white flex items-center">
                Bí kíp React Hooks cho người mới 
                <Badge className="ml-3 bg-green-500/20 text-green-400 border-none">Độc bản (Similarity: 12%)</Badge>
              </h2>
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" className="bg-white text-black hover:bg-gray-200">
                <Save className="w-4 h-4 mr-2" /> Lưu kịch bản
              </Button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto custom-scrollbar p-0">
            <div className="min-w-[800px]">
              ...
              <div className="grid grid-cols-[80px_1fr_2fr_1fr_1fr_40px] gap-2 p-3 border-b border-white/10 bg-[#1A1533]/50 text-xs font-semibold text-gray-400 sticky top-0 z-10 backdrop-blur-md">
                <div>THỜI GIAN</div>
                <div>CẢNH QUAY (VISUAL)</div>
                <div>LỜI THOẠI (AUDIO)</div>
              ...
              <div className="p-2 space-y-2">
                {scenes.map((scene, index) => (
                  <div key={scene.id} className="grid grid-cols-[80px_1fr_2fr_1fr_1fr_40px] gap-2 p-3 bg-[#1A1533]/30 rounded-lg border border-white/5 hover:border-purple-500/30 transition-colors group">
                    ...
                    <div className="space-y-2">
                      <Input defaultValue={scene.effect} className="h-8 bg-[#0F0B1A] border-white/10 text-pink-300 text-xs" title="Hiệu ứng hình ảnh" />
                      <Input defaultValue={scene.sound} className="h-8 bg-[#0F0B1A] border-white/10 text-cyan-300 text-xs" title="Âm thanh" />
                    </div>
                    
                    <div className="flex flex-col gap-1 items-center justify-start opacity-50 group-hover:opacity-100">
                      <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-400 hover:text-white"><ChevronUp className="w-4 h-4" /></Button>
                      <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-400 hover:text-white"><Unlock className="w-3 h-3" /></Button>
                      <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-400 hover:text-red-400"><Trash2 className="w-3 h-3" /></Button>
                      <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-400 hover:text-white"><ChevronDown className="w-4 h-4" /></Button>
                    </div>
                  </div>
                ))}
...
import { Settings2, Layout as LayoutIcon } from 'lucide-react';
